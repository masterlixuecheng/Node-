  ...
  <div class="bmgl">
    <el-form :inline="true" :model="queryForm">
      ...
      <el-form-item label="地区">
        <el-cascader :options="options" v-model="queryForm.areaList" change-on-select @change="handleItemChange" :props="props" clearable></el-cascader>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" @click="bm_search(1)">查询</el-button>
        <el-button type="success" icon="el-icon-plus" v-permission="'bmgl_add'" @click="isEdit = true;bmModal=true">添加角色</el-button>
    ...
    <el-table :data="tableData" border style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="deptname" label="角色名称"></el-table-column>
      <el-table-column prop="areaname" label="地区"></el-table-column>
      <el-table-column prop="description" label="备注"></el-table-column>
      <el-table-column prop="address" label="操作" width="420">
        <template slot-scope="scope">
          <el-button plain size="mini" type="success" @click="bm_detail(scope.row)">查看</el-button>
          <el-button plain size="mini" type="warning" v-permission="'bmgl_edit'" @click="bm_detail(scope.row,'edit')">修改</el-button>
          <el-button plain size="mini" type="danger" v-permission="'bmgl_del'" @click="bm_del(scope.row)">删除</el-button>
          <el-button plain size="mini" type="primary" v-permission="'bmgl_auth'" @click="bm_authManage(scope.row)">权限配置</el-button>
          <el-button plain size="mini" type="primary" v-permission="'bmgl_people'" @click="bm_account(scope.row)">关联用户</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <authModal :authModal="authModal" :id="authId" @close="close" @edit="bm_search"></authModal>
    <peoModal :modalShow="peoModal" :id="peoId" @close="close" @edit="bm_search"></peoModal>
  </div>
</template>
<script>
import { getBm, delBm } from "@/api/settings/bmgl";
import bmModal from "./bmgl_bm_modal";
import authModal from "./bmgl_auth_modal";
import peoModal from "./bmgl_people_modal";
