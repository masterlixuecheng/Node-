  ...
  <el-dialog title="角色" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
    <el-form :model="bmForm" ref="bmForm" label-width="100px" :rules="rules">
      ...
      <el-form-item label="区域">
        <el-cascader id="xzqh" :options="options" v-model="bmForm.areaList" change-on-select @change="handleItemChange" :props="props" clearable></el-cascader>
      </el-form-item>
...
import { addBm, setBm } from "@/api/settings/bmgl";
export default {
  name: "bmgl_bm_modal",
  data() {
    return {
  ...
  watch: {
    bmModal(val) {
      ...
      if(val){
        if(this.bmForm.areaList){
           this.getSuperiorXzqh(this.bmForm.areaList[this.bmForm.areaList.length - 1]);
        }else{
          this.getRegion();
        }
      }
