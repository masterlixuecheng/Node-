        ...
        <el-row>
          <el-col :span="10" :offset="1">
            <el-form-item class="no_label" label=".">
              <el-button type="primary" v-permission="'source_save'" @click="formSave">保存</el-button>
              <el-button type="info"  @click="$router.push({ name: 'mhxx' })">返回</el-button>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
<script>
// import { treeQuery, treeAdd, treeUpdate, treeDel, treeInsert, treeById} from "@/api/settings/mhxxsource";
import { postMethod, treeDel, formSave ,checkMhUri} from "@/api/settings/mhxxsource";
// import { formSave, } from "@/api/settings/source";
export default {
  name: "mhSource",
  data() {
    return {
  ...
  methods: { 
    checkUri(val) { 
      checkMhUri(this.formData.uri).then(res => {
        let data = res.data;
        if (!data.success) {
          this.formData.uri = "";
          this.$message({
    ...
    treeQuery() {
      let url = '/PermissionMh/findAllPermissionMh';
      let obj = {
        userid: this.userId 
      };
      if(this.$route.params.type == '2'){
        //app
        url = '/PermissionMh/findAllPermissionApp';
        obj.type = 2;
      }
    ...
    formSave() {
      this.$refs.treeForm.validate(valid => {
        if (valid) {
          ...
          this.openNodes = [this.parentId]; 
          if(this.$route.params.type == '2' && url == "add"){
            //app 
            obj.type = 2;
          }
